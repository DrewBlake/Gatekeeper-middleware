...
function gateKeeper(req, res, next) {
  const userNameHeader = req.get('x-username-and-password');
  const usernamePassObj = queryString.parse(userNameHeader);
  console.log(usernamePassObj);
  const result = USERS.find(function(element) {
    //console.log(usernamePassObj.user);
    return element.userName === usernamePassObj.user && element.password === usernamePassObj.pass;
  });
  //console.log(result);
  req.user = result;
  
  
  /*const passFound = USERS.find(function(element) {
    return element.password === usernamePassObj.password
  });*/
